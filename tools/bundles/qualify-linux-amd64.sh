...
"$prefix/bin/uv" workspace metadata --help >/dev/null
uv_smoke="$qualification_root/uv-smoke"
UV_CACHE_DIR="$qualification_root/uv-cache" "$prefix/bin/uv" venv \
	--offline --no-python-downloads --python "$prefix/bin/python3" "$uv_smoke"
"$uv_smoke/bin/python" -c 'import platform; assert platform.python_version() == "3.14.3"'
...
assert document["tools"]["uv"]["venvSmoke"] == "pass"
assert document["tools"]["uv"]["workspaceMetadata"] == "pass"
