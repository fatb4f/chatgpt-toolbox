...
repository_root=$(CDPATH='' cd -- "$bundle_root/../.." && pwd)
...
cue export \
	"$repository_root/environment/toolchain.cue" \
	"$bundle_root/toolchain-uv.cue" \
	-e bundle --out json > "$lock_json"
...
for key in sys.argv[2:]:
    ...
    if not isinstance(value, dict) or key not in value:
        raise SystemExit(f"canonical bundle lock missing path: {'/'.join(path)}")
    value = value[key]
print(value)
PY
}
...
uv_version=$(lock_value tools uv version)
uv_revision=$(lock_value tools uv revision)
uv_source=$(lock_value tools uv source)
uv_archive_sha256=$(lock_value tools uv sha256)
uv_target=$(lock_value tools uv target)
...
gopy_patch_file="$repository_root/$gopy_patch_path"
if [[ ! -f "$gopy_patch_file" ]]; then
	gopy_patch_file="$bundle_root/${gopy_patch_path##*/}"
fi
[[ -f "$gopy_patch_file" ]] || {
	echo "locked gopy patch unavailable: $gopy_patch_path" >&2
	exit 1
}
printf '%s  %s\n' "$gopy_patch_sha256" "$gopy_patch_file" |
	sha256sum --check --status
...
uv_archive="$work/download/uv-${uv_version}-${uv_target}.tar.gz"
download_verified "$uv_source" "$uv_archive_sha256" "$uv_archive"
mkdir -p "$work/uv-upstream"
tar -xzf "$uv_archive" -C "$work/uv-upstream"
uv_binary=$(find "$work/uv-upstream" -type f -name uv -perm -u+x -print -quit)
[[ -n "$uv_binary" ]] || { echo "uv archive did not contain an executable named uv" >&2; exit 1; }
[[ $("$uv_binary" --version) == "uv $uv_version ($uv_target)" ]] || {
	echo "uv executable identity does not match the canonical lock" >&2
	exit 1
}
...
make_stage() {
	...
	cp "$bundle_root/install.sh" "$stage/install.sh"
	cp "$bundle_root/verify_bundle.py" \
...
"$uv_binary" export --project "$repository_root" --locked --no-dev --no-emit-project \
	--format requirements.txt --output-file "$requirements"
"$uv_binary" pip install --python "$python_stage/libexec/python/bin/python3" \
...
uv_stage=$(make_stage uv)
cp "$uv_binary" "$uv_stage/bin/uv"
chmod 0755 "$uv_stage/bin/uv"
emit_component_manifest "$uv_stage" uv "$uv_version" "$uv_revision" "$uv_archive_sha256"
...
for name in python uv go cue gopls gopy; do
...
cp "$bundle_root/cuestrap_doctor.py" \
...
cp "$bundle_root/install-release.sh" "$output/install.sh"
...
printf 'file-count=%s\n' "$file_count"
printf 'tool-identities=uv:%s,cue:%s,gopls:%s,goimports:%s,gopy:%s\n' \
	"$uv_revision" "$cue_revision" "$gopls_revision" \
	"$goimports_revision" "$gopy_revision"
