...
def uv_smoke(prefix: Path) -> tuple[bool, str]:
    with tempfile.TemporaryDirectory(prefix="cuestrap-uv-doctor-") as temporary:
        root = Path(temporary)
        environment = os.environ.copy()
        ...
        if created.returncode:
            return False, (created.stdout + created.stderr)[-4000:]
        executed = run(
            [
                ...
                (
                    ...
                    + repr(json.loads((prefix / "share" / "cuestrap" / "manifest.json").read_text())["tools"]["python"]["version"])
                    + "; "
                    "assert pathlib.Path(sys.executable).is_relative_to("
                    + repr(str(root))
                    + ")"
                ),
            ],
        ...
        return executed.returncode == 0, (executed.stdout + executed.stderr)[-4000:]
...
def main() -> int:
    ...
    uv_ok, _ = probe(
        [str(prefix / "bin" / "uv"), "--version"], tools["uv"]["version"]
    )
    uv_workspace_ok, _ = probe(
        [str(prefix / "bin" / "uv"), "workspace", "metadata", "--help"],
        "workspace metadata",
    )
    uv_venv_ok, uv_diagnostic = uv_smoke(prefix)
    ...
    document = {
        ...
        "tools": {
            ...
            "uv": status(
                uv_ok and uv_workspace_ok and uv_venv_ok,
                version=tools["uv"]["version"],
                venvSmoke="pass" if uv_venv_ok else "fail",
                workspaceMetadata="pass" if uv_workspace_ok else "fail",
            ),
    ...
    diagnostics = {}
    if not uv_venv_ok:
        diagnostics["uv"] = uv_diagnostic
    if not gopy_build_ok:
        diagnostics["gopy"] = gopy_diagnostic
    if diagnostics:
